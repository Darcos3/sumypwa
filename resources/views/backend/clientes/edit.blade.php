                            ...
                            <div id="sortable" class="col-md-6">
                                @foreach ($atributo->terminos as $key => $termino)
                                    <div id="opcion{{$key}}"  class="input-group col-md-6">
                                        <input type="text" name="terminos_actuales[{{$termino->id}}]" class="form-control" value="{{$termino->nombre}}" aria-label="Recipient's username" aria-describedby="button-addon2">
                                        <button onclick="eliminarOpcionActual({{$key}},{{$termino->id}})" class="btn btn-outline-danger" type="button" id="nueva_opcion">Eliminar</button>
                                    </div>
                                @endforeach
                            </div>
                        </div>
    ...
    $( "#nueva_opcion" ).click(function() {
        ...
        $("#sortable").append(`
            <div id="opcion`+valor+`"  class="input-group col-md-6">
                ...
                <button onclick="eliminarOpcion(`+valor+`)" class="btn btn-outline-danger" type="button" id="nueva_opcion">Eliminar</button>
            </div>
        `);
